...
Component({
  ...
  methods: {
    openChat() {
      // 生成会话ID（首次打开或重新打开时生成）
      const sessionId = this.data.sessionId || 'session_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
      this.setData({ showChat: true, sessionId: sessionId });
    },
    closeChat() {
    ...
    handleStreamData(data, botMsgId) {
      ...
      switch (data.type) {
        ...
        case 'products':
          ...
          break;
        case 'guide_card':
          // 导购卡片 - 由 ShoppingGuideService 推送
          msg.guideCard = {
            guide_type: data.guide_type,
            guide_message: data.guide_message,
    ...
    onAddToCart(e) {
      ...
      wx.request({
        url: `http://localhost:8080/user/shoppingCart/add?productId=${id}&num=1`,
        method: 'POST',
        header: {
