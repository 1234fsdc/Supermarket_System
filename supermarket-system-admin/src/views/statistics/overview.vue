...
import { orderConditionSearch, getOrderStatistics, getProductSalesRankTop10 } from '@/api/order'
...
const initRankChart = async () => {
  ...
  try {
    const res = await getProductSalesRankTop10()
    const rankData = res.data || []

    const productNames = rankData.map(item => item.productName).reverse()
    const salesCounts = rankData.map(item => item.salesCount).reverse()
    ...
    chart.setOption({
      tooltip: {
        ...
        formatter: function(params) {
          const data = params[0]
          const item = rankData[rankData.length - 1 - data.dataIndex]
          return `${item.rank}. ${item.productName}<br/>销量：${item.salesCount} 件<br/>销售额：¥${item.salesAmount}`
        }
      },
      ...
      yAxis: {
        ...
        axisLabel: {
          ...
          formatter: function(value, index) {
            const rank = rankData.length - index
...
onMounted(() => {
  ...
  window.addEventListener('resize', () => {
    ...
    rankChartRef.value && echarts.getInstanceByDom(rankChartRef.value)?.resize()
  })
})
</script>
